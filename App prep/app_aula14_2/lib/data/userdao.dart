import 'package:app_aula14_2/data/database.dart';
...
class UsersDao {
  ...
  salvar(Users user) async {
    ...
    if (itemExist.isEmpty) {
      ...
      return await db
          .update(tabela, userMap, where: '$_nome =?', whereArgs: [user.nome]);
    }
  }
  ...
  procuraruser(String nomeusuario) async {
    ...
    final List<Map<String, dynamic>> result = await bd.query(tabela,
        where: '$_nome = ?',
  ...
  Future<List<Users>> listaruser() async {
    ...
    final List<Map<String, dynamic>> result =
        await bd.query(tabela); // pega todos os dados da tabela
    print("Resultado $result");
    return toList(result);
  }
